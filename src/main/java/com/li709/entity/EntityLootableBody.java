    ...
    public void onEntityUpdate() {
        ...
        if (ObjectUtils.notEqual(oldName, nameUpdate)) {
            ...
            if (nameUpdate != null && nameUpdate.trim().length() > 0) {
                ...
                //TODO 不读取玩家皮肤
                //				if(isClient) gp = TileEntitySkull.updateGameprofile(gp);
                setGameProfile(gp);
...
//        super.collideWithEntity(entityIn);
    ...
    protected boolean processInteract(EntityPlayer player, EnumHand hand) {
        if (player.getPosition().distanceSq(this.getPosition()) < 25) {
            FMLNetworkHandler.openGui(player, FlansMod.INSTANCE, 14, getEntityWorld(), getEntityId(), 0, 0);
            return true;
        }
        return false;
    }
    ...
    private void dropAllItems() {
        ...
        for (ItemStack i : mainInventory) {
            if (!i.isEmpty())
                this.getEntityWorld().spawnEntity(new EntityItem(this.getEntityWorld(), posX, posY, posZ, i));
        }
